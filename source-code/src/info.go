...
func HandleInfo(pkg string) {
	...
	for _, p := range repoPackages {
		if p.Name == pkg {
			...
			if p.Type == "lib" {
				usedBy := ReverseLibDependents(p.Name, p.Distro, repoPackages)
				if len(usedBy) > 0 {
					fmt.Printf("  %s  %s\n", BoldStyle.Render("Used by:"), strings.Join(usedBy, ", "))
				} else {
					fmt.Printf("  %s  %s\n", BoldStyle.Render("Used by:"), DimStyle.Render("no other catalog package currently depends on this"))
				}
			}
