...
func HandleInstall(pkg string, isolated bool, dryRun bool) {
	...
	case "de":
		...
		PrintInfo(fmt.Sprintf("Enter it with: isolator exec %s -- bash", pkg))
	case "lib":
		PrintInfo("This is a development library, not a runnable program — no wrapper script created.")
		PrintInfo(fmt.Sprintf("Build against it with: isolator exec %s -- <your build command>", pkg))
	case "system":
		if d.InitSystem == "systemd" {
			PrintInfo("This is a systemd-managed service, not a foreground program — no wrapper script created.")
			PrintInfo(fmt.Sprintf("Check it with: isolator exec %s -- systemctl status", pkg))
		} else {
			...
			if err := GenerateRcdScript(contName, pkg); err != nil {
				...
				PrintInfo(fmt.Sprintf("Control it with: isolator exec %s -- /etc/rc.d/rc.%s {start|stop|restart|status}", pkg, pkg))
			}
		}
	default:
